  ...
  router.route("/test").get(function (req, res) {
    ...
    var email = {
      body: {
        name: "Amit Chauhan",
        link: ` `,
        intro: "We have received your enquiry. We will contact you within next 24 hours.",
        action: {
    ...
    mail.sendMail("amitchauhan6dec@gmail.com", "Welcome to Arab Tech Store", emailBody);
          ...
          if (err) {
            ...
            enquiry.save((err) => {
              if (err) {
                ...
                // Generate an HTML email with the provided contents
                var emailBody = mailGenerator.generate(email);
                mail.sendMail(req.body.email, "Welcome to Arab Tech Store", emailBody);
                response.error = false;
                response.status = 200;
                response.errors = null;
                response.data = null;
...
methods.getClients = async (req, res) => {
  if (req.query.clientId) {
    ...
    if (req.query.startDate && req.query.endDate) {
      let createdAtGTE = new Date(moment(req.query.startDate).utc("0530").format());
      let createdAtLTE = new Date(moment(req.query.endDate).utc("0530").format());
      query["$and"].push({
        createdAt: {
          $gte: createdAtGTE,
          $lte: createdAtLTE,
...
methods.deactivateClientId = function (req, res) {
  ...
  if (errors) {
    ...
    Client.findOneAndUpdate({ _id: req.body.clientId }, { active: 0 }, function (err) {
      if (err) {
        //send response to client
        response.error = true;
        response.status = 500;
